...
namespace Routing.Web.Tests
{
    ...
    public class ControllerNavigatorTests
    {
        ...
        {
            var correctTypes = Assembly
                .GetExecutingAssembly()
                .GetTypes()
                .Except(new List<Type>() {typeof (TestRoot2Controller)});
        ...
        public void AssertTwoRootControllersThrows()
        {
            Assert.Throws(typeof(InvalidOperationException),
                () =>
                ControllerNavigator
				.TransformControllersToRouteInfo(Assembly.GetExecutingAssembly(), typeof(TestController), "Routing.Web.TestTwoRootHierarchy"));
        }

		[Test]
		public void AssertRootControllerGoesFirst()
		{
			var routeRoot = ControllerNavigator.TransformControllersToRouteInfo(Assembly.GetExecutingAssembly(), typeof (TestController), "Routing.Web.TestHierarchy");
			...
			Assert.That(routeRoot.Children.First().Children.First().Controller, Is.EqualTo(typeof(ResourcesController).Name));
